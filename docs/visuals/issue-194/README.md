...
| Style behavior | Text-subtitle presets apply live. High contrast supplies a semi-transparent background box; ASS/SSA retains authored styling. | All presets write the same seven mpv fields, so switching clears prior state. `Contrast` uses `background-box` with 72% black; real-libmpv tests verify the property names and values. |

...
impl Settings {
    ...
    pub fn load(raw: &str) -> Option<Self> {
        ...
        if is_native {
            ...
            if settings.video.hwdec.is_none() {
                settings.video.hwdec = default_hwdec_setting();
            }
            Some(settings)
        } else if is_windows {
...
mod tests {
    ...
    fn native_document_without_hwdec_defaults_to_auto_safe() {
        ...
        let null_hwdec = Settings::load(r#"{ "version": 2, "video": { "hwdec": null } }"#)
            .expect("document with null hwdec should load");

        assert_eq!(missing_video.video.hwdec.as_deref(), Some("auto-safe"));
        assert_eq!(missing_hwdec.video.hwdec.as_deref(), Some("auto-safe"));
        assert_eq!(null_hwdec.video.hwdec.as_deref(), Some("auto-safe"));
    }

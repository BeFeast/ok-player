...
public sealed class UpdateService
{
    ...
    {
        // The update feed is the static releases.win.json on GitHub Pages (UpdateFeed.WinBaseUrl), NOT the
        ...
        // The static manifest is regenerated by the release pipeline on every published Windows release and
        // always lists the newest (pre-)release builds, so the old prerelease:true semantics carry over — the
        // pre-1.0 betas keep being offered. Packages still download from GitHub release assets via the
        // absolute URLs inside the manifest (SimpleWebSource downloads URL-valued entries as-is).
        ...
        var source = new SimpleWebSource(UpdateFeed.WinBaseUrl);
        _mgr = new UpdateManager(source, new UpdateOptions { ExplicitChannel = UpdateFeed.WinChannel });
    }

...
//! carry their own embedded styling. The engine adapter pins `sub-ass-override=scale` and sets the
//! secondary equivalent when the linked libmpv exposes it. This deliberately preserves authored
//! fonts, colors, inline layout, and signs while still allowing OK Player's explicit size/position
//! controls. A preset therefore does NOT repaint ASS/SSA — the same asymmetry that forced the OSC
//! lift onto `sub-pos` rather than `sub-margin-y` (see [`crate::subtitle_lift`]). Every preset sets
//! the SAME set of options, so switching presets fully overrides the previous one with no residual
//! state.

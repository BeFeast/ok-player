FROM ubuntu@sha256:4fbb8e6a8395de5a7550b33509421a2bafbc0aab6c06ba2cef9ebffbc7092d90 AS media

ENV DEBIAN_FRONTEND=noninteractive
ENV PATH=/root/.cargo/bin:${PATH}

COPY linux-candidate-toolchain.manifest /tmp/linux-candidate-toolchain.manifest
COPY linux-candidate-toolchain.sh /tmp/linux-candidate-toolchain.sh
...
FROM media AS appimage

ENV DOTNET_ROOT=/opt/dotnet
ENV PATH=/opt/dotnet:/root/.dotnet/tools:${PATH}

RUN curl --proto '=https' --tlsv1.2 -sSfL https://dot.net/v1/dotnet-install.sh \

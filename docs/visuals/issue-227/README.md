...
CC=/usr/bin/cc cargo test -p okp-mpv ass_override_boundary_preserves_authored_styles_with_legacy_fallback
```
...
proves the primary override and either the secondary override or the intentional legacy fallback;
authored-script rendering remains mpv/libass's native path.

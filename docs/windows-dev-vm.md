...
csproj — the report cannot disagree with what the build restores. `overall: ok` means every required tool is
present and at or above its baseline (CMake/Ninja are optional and never fail the report).
